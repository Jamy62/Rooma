<?php 
    ...
    if ($count > 0)
    {
        echo "<script>window.alert('This account verification has already been submitted or has already been registered as a Guest.')</script>";
	...
	if (isset($_POST['btnVerify'])) 
	{
		...
		$insert = "INSERT INTO verifications(UserAccountType, LegalName, Selfie, IDNumber, IDFront, IDBack, IDType, VerificationDate, VerificationStatus, UserID, CountryID)
				   VALUES('Guest','$legalName', '$selfieFileName', '$idNumber', '$idFrontFileName', '$idBackFileName', '$idType', '$verificationDate', '$verificationStatus', '$uID', '$cID')";

		$verificationInsert = mysqli_query($dbConnect, $insert);

		if ($verificationInsert)
		{
			echo "<script>window.alert('Guest account verification application has been submitted. We will reach back to you once your account is verified.')</script>";
            echo "<script>window.location='guestMain.php'</script>";
		}
	}
 ?>
...
<body id="RegisterBody">
	<div id="registerContainer">
	    <form id="Register" action="guestVerify.php" method="POST" enctype="multipart/form-data">
